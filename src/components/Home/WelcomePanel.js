...
export default function WelcomePanel() {
    return (
        <div style={{backgroundImage: `url(${beckgroungTransperant})`}}>
            ...
            <p style={{fontSize: 14}} className="pt-running-text" class='center'>The application operates as a three steps pipeline: (1) the user uploads his data using two files - a cytokines measurements file and a clinical measurements outcome file (not mandatory), (2) the user specifies the cytokines and outcomes he wishes to analyze, and can also tune the default statistical parameters according to his specific research (number of clusters K, log transformations,  covariate variables) and (3) The method is activated and the user can view and download the results. The method activation is done by pressing a single button and the results are kept with a unique ID for a week, so the user can visit the results again, without the need to re-run the analysis. The results and data cannot be accessed by other users. In addition, the method can be executed without any outcome variables, so only cytokines modules will be discovered without clinical associations.</p>
            ...
            <p style={{fontSize: 14}} className="pt-running-text" class='center'>Main results presented by the CytoMod server: (Left) The clustering tab. Clustering results (upper figure) are presented using the correlation matrix between cytokines, as well as the dendrogram produced in the clustering process.  Clusters (i.e. cytokine modules) are presented as different colors on the y-axis color bar. The lower figure demonstrates the amount of times that each pair of cytokines were clustered together. (Center) The module inner-correlation tab includes images presenting the correlation within each created module. The image shown presents inner correlations inside module 1 from the left image. (Right) The outcome analysis tab. Associations are shown between cytokines and cytokine modules and the clinical phenotypes. The heatmap colors indicates the direction and magnitude of the regression coefficient. Only associations with false-discovery rate (FDR)-adjusted q-value ≤ 0.2 are colored. Asterisks indicate family-wise error rate (FWER)-adjusted p-values with ***, **, and * indicating p ≤ 0.0005, 0.005, and 0.05, respectively. The user can set analysis hyper parameters (like number of clusters, or execution of log-transformation pre-processing) and see how they affect analysis results.
</p>
